/**
 * WorkerPoolManager - Web Worker Pool for Parallel ML Processing
 * Renamed from MLWorkerPool, extracted from agents_output/wave3/optimization/
 * 
...
class WorkerPoolManager {
    constructor(config = {}) {
        ...
        this.performanceMonitor = window.KC?.ML?.MLPerformanceMonitor || { startTimer: () => ({ end: () => {} }), startMeasure: () => {}, endMeasure: () => 0 };
    ...
    async _initializeWorkers() {
        if (!this.workersSupported) {
            console.warn('WorkerPoolManager: Web Workers not supported, falling back to main thread');
            return;
        }
        ...
        try {
            ...
            console.log(`WorkerPoolManager: Initialized ${this.workers.length} workers`);
            
        } catch (error) {
            console.error('WorkerPoolManager: Failed to initialize workers', error);
    ...
    async createWorker(id) {
        try {
            ...
            console.error(`WorkerPoolManager: Failed to create worker ${id}`, error);
            return null;
        }
    }
...
`;
    ...
    handleWorkerError(worker, error) {
        console.error(`WorkerPoolManager: Worker ${worker.id} error`, error);
    ...
    async replaceWorker(failedWorker) {
        ...
        try {
            ...
            console.error('WorkerPoolManager: Failed to replace worker', error);
        }
    }
    ...
    async executeOnMainThread(taskType, data) {
        console.warn('WorkerPoolManager: Executing on main thread (fallback)');
    ...
    async warmup() {
        ...
        try {
            ...
            console.log('WorkerPoolManager: Warmup complete');
        } catch (error) {
            console.warn('WorkerPoolManager: Warmup failed', error);
        }
    }
    ...
    async terminate() {
        ...
        console.log('WorkerPoolManager: Terminated');
    }
}

// Register with KC
if (window.KC) {
    window.KC.WorkerPoolManager = WorkerPoolManager;
    console.log('[WorkerPoolManager] Registered with KC');
}