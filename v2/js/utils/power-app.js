...
class PowerApp {
    ...
    const html = filteredFiles.map(file => `
      <div class="file-item" data-file-id="${file.id}">
        ...
        <div class="file-relevance">
          <span>${Math.round(file.relevance)}%</span>
          <div class="relevance-bar">
            <div class="relevance-fill" style="width: ${file.relevance}%"></div>
          </div>
        </div>
        <div class="file-date">${this.formatDate(file.modified)}</div>
        <div class="file-status">
          <span class="status-badge ${file.analyzed ? 'analyzed' : ''}">
  ...
  generateMockFiles(count) {
    const files = [];
    const names = ['project-plan', 'meeting-notes', 'technical-spec', 'user-story', 'design-doc'];
    const extensions = ['.md', '.txt', '.pdf', '.docx'];
    
    for (let i = 0; i < count; i++) {
      files.push({
        id: `file-${i}`,
        name: `${names[i % names.length]}-${i}${extensions[i % extensions.length]}`,
        path: `/documents/project-${Math.floor(i / 10)}/${names[i % names.length]}-${i}${extensions[i % extensions.length]}`,
