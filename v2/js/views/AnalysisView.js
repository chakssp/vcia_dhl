...
export class AnalysisView {
  ...
  updateProgress(data) {
    ...
    if (data.completed !== undefined) this.progress.completed = data.completed;
    if (data.failed !== undefined) this.progress.failed = data.failed;
    if (data.pending !== undefined) this.progress.pending = data.pending;
  ...
  updateProgressDisplay() {
    ...
    if (progressBar && this.progress.total > 0) {
      const percentage = Math.round((this.progress.completed / this.progress.total) * 100);
      progressBar.style.width = `${percentage}%`;
    }
    
    if (progressText) {
      progressText.textContent = `${this.progress.completed} / ${this.progress.total} files analyzed`;
    }
  }
  ...
  updateProviderDisplay() {
    // Implementation will update provider status UI
    ...
    providerElements.forEach(el => {
      ...
      if (provider) {
        el.classList.toggle('online', provider.status === 'online');
        el.classList.toggle('offline', provider.status === 'offline');
        el.classList.toggle('error', provider.status === 'error');
      }
    });
  }
